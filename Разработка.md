...
Для каждой биржи необходимо реализовать:

- [ ] `Connect(apiKey, secret string) error` - подключение и проверка ключей
- [ ] `GetBalance() (float64, error)` - получение equity в USDT
- [ ] `GetTicker(symbol string) (Ticker, error)` - текущая цена
- [ ] `GetOrderBook(symbol string, depth int) (OrderBook, error)` - стакан ордеров
...
- [ ] `SubscribeTicker(symbol string, callback func(Ticker))` - WebSocket подписка
...
- [ ] Обработка ошибок (sql.ErrNoRows, etc.)
- [ ] Транзакционная поддержка где необходимо
- [ ] Тесты
...
- [ ] Тестовое подключение при ConnectExchange
- [ ] Остановка пар при DisconnectExchange
...
**Функциональность:**
- [ ] Главный event loop (каждую секунду)
- [ ] Обработка всех активных пар
- [ ] Координация между компонентами
- [ ] Периодическое обновление балансов (каждую минуту)
- [ ] Graceful shutdown
- [ ] Соблюдение лимита max_concurrent_trades
...
- [ ] Расчет PNL по каждой ноге: `(P_current - P_entry) × qty`
...
**Функциональность:**
- [ ] Мониторинг Stop Loss: `total_PNL ≤ -SL`
...
- Реализация логики: ⏳ Требуется имплементация

**Frontend (React):**
- Структура файлов: ❌ 0% (директория не создана)
- Реализация: ❌ Не начато

**Следующий шаг:** Начать с Фазы 2 - реализация SQL запросов в репозиториях и логики в сервисах.
