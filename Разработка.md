...
> **✅ Аудит пройден (2025-12-04):**
> - Добавлен метод `Unwrap()` к `ExchangeError` для поддержки `errors.Is()`/`errors.As()`
> - Улучшена документация констант Side (разделены на orders и positions)
...
> **✅ Аудит Bybit пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket managers (добавлен wsMu mutex)
> - Добавлено логирование ошибок парсинга (parseFloat/parseInt/parseInt64)
> - Удалено дублирование кода в doRequest

> **✅ Аудит Bitget пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket managers (добавлен wsMu mutex)
> - Добавлено логирование ошибок парсинга (parseFloat/parseInt/parseInt64)
> - GetTradingFee возвращает хардкод 0.0004 (допустимо, стандартная комиссия Bitget)

> **✅ Аудит OKX пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket managers (добавлен wsMu mutex)
> - Добавлено логирование ошибок парсинга (parseFloat/parseInt/parseInt64)
> - GetTradingFee возвращает хардкод 0.0005 (допустимо, стандартная комиссия OKX)

> **✅ Аудит Gate.io пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket manager (добавлен wsMu mutex)
> - Добавлено логирование ошибок парсинга (parseFloat)
> - GetTradingFee возвращает хардкод 0.0005 (допустимо, стандартная комиссия Gate.io)

> **✅ Аудит HTX пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket manager (добавлен wsMu mutex)
> - HTX API возвращает float64 напрямую (не строки) - parseFloat не требуется
> - GetTradingFee возвращает хардкод 0.0004 (допустимо, стандартная комиссия HTX)

> **✅ Аудит BingX пройден (2025-12-04):**
> - Исправлен race condition при инициализации WebSocket manager (добавлен wsMu mutex)
> - Добавлено логирование ошибок парсинга (parseFloat)
> - GetTradingFee возвращает хардкод 0.0005 (допустимо, стандартная комиссия BingX)
...
- [x] `SubscribePositions(callback func(Position))` - WebSocket подписка на позиции
- [x] `GetTradingFee(symbol string) (float64, error)` - комиссия тейкера
- [x] `GetLimits(symbol string) (Limits, error)` - лимиты биржи (min/max)
- [x] `Close() error` - закрытие соединений
- [x] `GetName() string` - имя биржи

> **✅ Аудит 3.3 пройден (2025-12-04):**
> - Все 13 методов интерфейса Exchange реализованы для всех 6 бирж
> - **Исправлено:** Gate.io handleMessage теперь обрабатывает канал `futures.positions`
> - **Документировано:** HTX/BingX SubscribePositions используют REST API polling (WebSocket не реализован)
> - GetTradingFee для Bitget/OKX/Gate/HTX/BingX возвращает хардкод (допустимо)
> - Factory корректно создаёт экземпляры всех бирж
...
> **✅ Аудит 4.1-4.2 пройден (2025-12-04):**
> - Все 7 репозиториев реализуют полный CRUD с корректной обработкой ошибок
> - SQL injection защита: все запросы параметризованы ($1, $2, ...)
> - **Исправлено:** NotificationRepository.GetByTypes - некорректная генерация placeholders для 10+ типов
> - defer rows.Close() и rows.Err() проверяются везде
> - JSON сериализация для сложных полей (Meta, NotificationPrefs)
> - Автоматическое создание defaults в SettingsRepository
...
> **✅ Аудит 5.1-5.2 пройден (2025-12-04):**
> - Все 7 сервисов реализованы: Exchange, Pair, Stats, Notification, Blacklist, Settings
> - **Исправлено:** ExchangeService.connections защищён sync.RWMutex от race condition
> - RiskService реализован в bot/risk.go (745 строк) как RiskManager + RiskMonitor
> - Документирована архитектура: risk_service.go → ссылка на bot.RiskManager
> - Интерфейсы для всех репозиториев и 4 сервисов (interfaces.go)
> - WebSocket broadcast интегрирован во все сервисы
...
> **✅ Аудит ExchangeHandler пройден (2025-12-04):**
> - **Исправлено:** Добавлен `ExchangeServiceInterface` для Dependency Injection
> - **Исправлено:** Добавлено ограничение размера тела запроса `http.MaxBytesReader` (1 MB)
> - **Исправлено:** Удалён избыточный `defer r.Body.Close()`
> - **Исправлено:** Ошибка `w.Write()` теперь явно игнорируется (`_, _ = w.Write()`)
> - Все 4 endpoints реализованы корректно

> **✅ Аудит PairHandler пройден (2025-12-04):**
> - **Исправлено:** Добавлен `PairServiceInterface` для Dependency Injection
> - **Исправлено:** Добавлено ограничение размера тела запроса в CreatePair и UpdatePair
> - **Исправлено:** Ошибка `json.Encode()` теперь явно игнорируется
> - Все 7 endpoints реализованы корректно
> - Отличная обработка ошибок (15+ типов через handleServiceError)
> - Поддержка отложенных изменений при открытой позиции

> **✅ Аудит NotificationHandler пройден (2025-12-04):**
> - Handler уже использует интерфейс `NotificationServiceInterface`
> - Все 2 endpoints реализованы корректно
> - Фильтрация по типам и лимиту

> **✅ Аудит StatsHandler пройден (2025-12-04):**
> - Handler уже использует интерфейс `StatsServiceInterface`
> - Все 3 endpoints реализованы корректно
> - Защита от null массивов в JSON ответе

> **✅ Аудит BlacklistHandler пройден (2025-12-04):**
> - Handler уже использует интерфейс `BlacklistServiceInterface`
> - **Исправлено:** Добавлено ограничение размера тела запроса `http.MaxBytesReader`
> - **Исправлено:** Ошибка `json.Encode()` теперь явно игнорируется
> - **Исправлено:** Используется стандартный `ErrorResponse` вместо `map[string]string`
> - Все 3 endpoints реализованы корректно
> - Типизированная обработка ошибок через `errors.Is()`
...
>
> **Реализовано (2025-12-03) - Централизованный RiskManager:**
> - **RiskManager** (`internal/bot/risk.go`): полный менеджер рисков
> - **CheckStopLoss / HandleStopLoss**: мониторинг и обработка Stop Loss (PNL ≤ -SL)
> - **HandleLiquidation**: обработка ликвидаций с экстренным закрытием второй ноги
> - **emergencyCloseLeg**: экстренное закрытие с aggressive retry (4 попытки)
> - **CheckMarginRequirement / CheckBothLegsMargin**: проверка маржи для обеих ног
> - **ValidateOrderLimits**: валидация лимитов биржи (min/max qty, minNotional)
> - **RiskMonitor**: воркер для периодической проверки рисков (каждые 500ms)
> - **Кэширование**: marginCache и limitsCache через sync.Map
> - **Уведомления**: notifyStopLoss, notifyLiquidation, notifyMarginInsufficient, notifyError
> - **Переход HOLDING → PAUSED**: добавлен в state_machine.go для SL/ликвидаций
...
| `[ ]` | Индикатор статуса WebSocket | `frontend/src/components/Common/Header.tsx` | Отображение статуса соединения |
...
|--------|--------|----------|
| `[x]` | Тесты моделей | Валидация структур данных (models_test.go: 40 тестов + 4 бенчмарка) |
...
| `[x]` | Тесты handlers | Mock сервисов, HTTP запросы (blacklist, settings, notification, stats handlers) |
...
| Статус | Задача | Описание |
|--------|--------|----------|
| `[x]` | API integration tests | Полный цикл HTTP запросов (tests/integration/api_test.go) |
| `[x]` | WebSocket tests | Подключение, broadcast, типы сообщений (tests/integration/websocket_test.go) |
| `[x]` | Database tests | Schema, CRUD, транзакции, concurrent access (tests/integration/database_test.go) |
...
- [ ] Frontend создан и собирается для production
...
**Frontend (React):**
- Структура файлов: ❌ Не создано
- Базовая структура: ❌ Не создано (package.json, tsconfig.json, типы)
- Сервисы: ❌ Не создано (API клиент, WebSocket клиент)
- Хуки: ❌ Не создано (usePairs, useExchanges, useWebSocket, useNotifications)
- Общие компоненты: ❌ Не создано (Header, Navigation, BalanceBar)
- Вкладка БОТ: ❌ Не создано (BotPage, PairCard, AddPairModal, EditPairModal, Blacklist)
- Вкладка API/STATS: ❌ Не создано (APIStatsPage, ExchangeList, ExchangeCard, StatsOverview, TopPairs, EventsList)
- Вкладка Уведомления: ❌ Не создано (NotificationsPage, NotificationFeed, NotificationFilters)
- Главное приложение: ❌ Не создано (App с роутингом, index.tsx)
- Интеграция WebSocket: ❌ Не создано (real-time обновления)
...
- Models (internal/models): ✅ 100% покрытие (models_test.go: 40 тестов + 4 бенчмарка)
...
- Handlers (internal/api/handlers): ✅ Unit тесты (blacklist, settings, notification, stats handlers)

**Интеграционные тесты:**
- API integration tests: ✅ 100% реализовано (tests/integration/api_test.go)
- WebSocket tests: ✅ 100% реализовано (tests/integration/websocket_test.go)
- Database tests: ✅ 100% реализовано (tests/integration/database_test.go)

**Следующий шаг:** Создание Frontend (React) с нуля по документации Архитектура.md
