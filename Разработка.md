...
```go
// 1. Создаем и запускаем WebSocket Hub
wsHub := websocket.NewHub()
go wsHub.Run()

// 2. Подключаем Hub к сервисам для broadcast
notificationService.SetWebSocketHub(wsHub)
exchangeService.SetWebSocketHub(wsHub)
statsService.SetWebSocketHub(wsHub)

// 3. Передаем Hub в Engine при создании
engine := bot.NewEngine(cfg, wsHub)
```
...
### 8.9 Recovery (Восстановление) ✅ ВЫПОЛНЕНО
...
**Следующий шаг:** Unit/интеграционные тесты (Приоритет 10) или API документация (Приоритет 11)

---

## История оптимизаций

### 2025-12-02: Recovery Manager (Приоритет 8.9)

**Реализовано:**

1. **RecoveryManager** (`internal/bot/recovery.go`)
   - Полное восстановление работы бота после перезапуска сервера
   - Параллельное подключение к биржам с расшифровкой API ключей
   - Обнаружение открытых позиций на всех биржах (параллельно)
   - Сопоставление позиций с парами бота по символу
   - Восстановление runtime состояния (HOLDING) для найденных позиций

2. **Ключевые методы:**
   - `Recover(ctx)`: синхронное восстановление с возвратом результата
   - `RecoverAsync(ctx)`: асинхронное восстановление через канал
   - `VerifyPositions(ctx)`: проверка согласованности engine ↔ биржи
   - `ClosePositionManually(ctx, exchange, symbol, side, qty)`: ручное закрытие

3. **Обработка граничных случаев:**
   - Неполные позиции (одна нога) → состояние ERROR
   - "Потерянные" позиции → уведомление пользователя
   - Опциональное автоматическое закрытие orphaned позиций
   - Ошибки подключения → продолжение с оставшимися биржами

4. **Структуры данных:**
   - `RecoveryResult`: полные результаты процесса восстановления
   - `DiscoveredPosition`: найденная позиция на бирже
   - `MatchedPosition`: позиция, связанная с парой бота
   - `RecoveryConfig`: конфигурация восстановления

### 2025-12-02: Arbitrage Logic (Приоритет 8.5, 8.3, 8.8)

**Реализовано:**

1. **ArbitrageDetector** (`internal/bot/arbitrage.go`)
   - Определение арбитражных возможностей (O(1) через PriceTracker)
   - DetectOpportunity / DetectWithLiquidity: обнаружение с учётом ликвидности
   - Кэш маржинальных требований (marginCache)
   - Метрики: opportunitiesDetected, entriesTriggered, exitsTriggered

2. **EntryConditionChecker** (`internal/bot/arbitrage.go`)
   - CheckEntryConditions: полная проверка условий входа
   - Проверка спреда: net_spread >= entry_spread
   - Проверка ликвидности через OrderBookAnalyzer
   - Проверка маржи (checkMarginRequirement)
   - Проверка лимитов ордеров (OrderValidator)
   - Проверка максимального количества арбитражей

3. **ExitConditionChecker** (`internal/bot/arbitrage.go`)
   - CheckExitConditions: проверка условий выхода
   - Проверка спреда: current_spread <= exit_spread
   - Проверка StopLoss: PNL <= -SL
   - ExitReason: spread_reached, stop_loss, liquidation, manual, error

4. **PartialEntryManager** (`internal/bot/arbitrage.go`)
   - Частичный вход в позицию (N ордеров)
   - Проверка спреда перед каждой частью (80% от entry_spread)
   - Аккумуляция результатов (VWAP для всех частей)
   - Остановка при ухудшении спреда

5. **SecondLegFailHandler** (`internal/bot/arbitrage.go`)
   - Обработка провала второй ноги
   - Откат успешной первой ноги
   - Генерация уведомления SECOND_LEG_FAIL
   - Логирование события

6. **ArbitrageCoordinator** (`internal/bot/arbitrage.go`)
   - Координация всех арбитражных операций
   - TryEnter: попытка входа с проверкой условий
   - TryExit: попытка выхода с проверкой условий

7. **Интеграция в Engine** (`internal/bot/engine.go`)
   - checkArbitrageOpportunity: использует ArbitrageDetector
   - executeEntryWithConditions: выполнение входа с условиями
   - executeExit: закрытие позиции с уведомлениями
   - exitConditionChecker: периодическая проверка выхода (500ms)
   - notificationWorker: обработка уведомлений
   - updatePairPnl: расчёт PNL и текущего спреда

8. **State Machine переходы** (`internal/bot/engine.go`)
   - PAUSED → READY (API start)
   - READY → ENTERING (условия входа)
   - ENTERING → HOLDING (успешный вход)
   - ENTERING → READY (откат)
   - HOLDING → EXITING (условия выхода)
   - HOLDING/EXITING → PAUSED (SL/ликвидация)
   - EXITING → READY (успешное закрытие)
   - → ERROR (ошибка закрытия)

### 2025-12-02: Spread Calculator и Order Validator (Приоритет 8.4, 8.6)

**Реализовано:**

1. **OrderBookAnalyzer** (`internal/bot/spread.go`)
   - Анализатор стаканов ордеров (5 уровней глубины)
   - Кэширование стаканов с проверкой актуальности (maxAge)
   - Моделирование исполнения рыночных ордеров (SimulateBuy/SimulateSell)
   - Расчёт средневзвешенной цены (VWAP) для заданного объёма
   - Оценка slippage относительно лучшей цены
   - Полный анализ ликвидности для арбитража (AnalyzeLiquidity)
   - Быстрая проверка достаточности ликвидности (CheckLiquidityForVolume)

2. **Расширение SpreadCalculator** (`internal/bot/spread.go`)
   - GetRealSpread: расчёт спреда с учётом VWAP и slippage
   - GetSpreadWithLiquidity: спред с информацией о ликвидности
   - Интеграция с OrderBookAnalyzer

3. **OrderValidator** (`internal/bot/order.go`)
   - Валидация min/max qty для каждой биржи
   - Округление до lot size (qtyStep)
   - Проверка minNotional (минимальная сумма сделки)
   - Кэширование лимитов по биржам (sync.Map)
   - ValidateBothLegs: валидация обеих ног арбитража
   - PreloadLimits: предзагрузка лимитов для всех пар

4. **ExecuteWithRetry** (`internal/bot/order.go`)
   - Интеграция с pkg/retry для повторных попыток
   - Определение retriable ошибок (сетевые, rate limit)
   - Exponential backoff при ошибках

**Оптимизации:**
- sync.Pool для LegResultChan (убрали аллокации каналов)
- struct keys вместо string concatenation (PriceKey, OrderBookKey, LimitsKey)
- sync.Map для lock-free кэширования
...
### 2025-12-XX: Playwright e2e тесты (Приоритет 10.3)

**Реализовано:**

1. **Инфраструктура тестирования**
   - Добавлен Playwright (`@playwright/test`) в devDependencies
   - Настроен `playwright.config.ts` с автоматическим запуском Vite dev-сервера
   - Добавлены npm-скрипты: `test:e2e`, `test:e2e:ui`, `test:e2e:report`

2. **Сценарии пользователя**
   - Тест `tests/e2e/botPage.spec.ts` покрывает:
     * Добавление новой пары через модальное окно (`AddPairModal`)
     * Запуск и паузу арбитражной пары (кнопки Старт/Пауза)
   - Реализован утилитный мок-сервер (`tests/e2e/utils/mockApi.ts`) с перехватом REST-запросов (`/api/pairs`, `/api/exchanges`)
   - Тесты работают без реального backend'а, имитируя ответы API и проверяя основные пользовательские флоу

3. **Документация**
   - Обновлен раздел 10.3 `Разработка.md`: задачи «Cypress/Playwright» и «Сценарии пользователя» отмечены как выполненные
   - Добавлено описание новых тестов в истории изменений
